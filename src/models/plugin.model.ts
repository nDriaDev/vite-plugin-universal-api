...
/**
 * Authentication option for REST and WebSocket handlers.
 *
 * Possible values:
 * - `false` — No authentication check (default).
 * - `true`  — Verifies that the `authorization` header is present and non-empty.
 * - `string` — Verifies that the specified header is present and non-empty.
 * - `function` — Custom async-capable function; receives the request and must return
 *   `true` to allow or `false` to reject.
 *
 * When authentication fails, the server responds with `401 Unauthorized`.
 * If the function throws, the server responds with `500 Internal Server Error`.
 *
 * @example
 ...
 * }
 */
export type UniversalApiAuthenticate =
	| false
	| true
	| string
...
type UniversalApiHandlerCommon = {
	 ...
	 * - `function` — Custom predicate; return `true` to allow, `false` to reject.
	 *
	 * A rejected request receives a `401 Unauthorized` response.
	 * If a function throws, the response is `500 Internal Server Error`.
	 *
	 * @default false
	 *
	 * @example
...
export type UniversalApiWsHandler = {
	 ...
	 *
	 * Accepts the same values as the REST handler `authenticate` option (see {@link UniversalApiAuthenticate}):
	 *
	 * - `false` — No check (default).
	 * - `true`  — The `authorization` header must be present and non-empty.
	 * - `string` — The specified header must be present and non-empty.
	 * - `function` — Custom predicate receiving the HTTP upgrade request; return `true` to
	 *   allow the connection, `false` to reject it with `401 Unauthorized`.
	 *   Can be async for database lookups, token validation, etc.
	 *
	 * @throws If a function is provided and throws, the connection is rejected with `500 Internal Server Error`.
	 *
	 * @default false
	 *
	 * @example
